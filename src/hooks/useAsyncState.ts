...
type SetStateAction<S> = (prevState: S | ((prevState: S) => S | Promise<S>)) => Promise<S>;
...
export default function useAsyncState<S>(init: S | (() => S)): [S, SetStateAction<S>] {
  ...
  const asyncSetState: SetStateAction<S> = useCallback(async (value: S | ((prevState: S) => S | Promise<S>)) => {
